...
def benchmark_fun(w_pl, w_ph, w_pc, td_year, td_month, td_day, ref_data, ref_fname, tar_fname):
    ...
    log_fname = "plots/" + str(td_year) + str(td_month) + str(td_day) + "-" + \
               str(w_pl) + "-" + str(w_ph) + "-" + str(w_pc) + ".txt"
    dat_fame = "plots/" + str(td_year) + str(td_month) + str(td_day) + "-" + \
               str(w_pl) + "-" + str(w_ph) + "-" + str(w_pc) + ".csv"
    ...
    for tar_tick in progressbar.progressbar(tar_ticks, widgest=widgets):
        ...
        if(gainz[1] < target['percent_high'].iloc[tar_end_i].to_numpy()):
            ...
            roi1 = target['percent_close'].iloc[tar_end_i].to_numpy()
            roi = roi1[0]
